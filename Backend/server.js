...
app.use("/Spot-RM-PM_inspection_pdfs", express.static(path.join(__dirname, "Spot-RM-PM_inspection_pdfs" )));
...
app.post('/spot-rm-pm', (req, res) => {
  ...
  const sql = `
    INSERT INTO spot_RM_PM_vehicle_form (
      docket_no, dmc_in_datetime, dmc_out_datetime,
      truck_number, vehicle_type, transporter,
      driver_safety_induction, driver_counselling,
      vehicle_documents, vehicle_documents_remark,
  ...
  db.beginTransaction((txErr) => {
    ...
    db.query(sql, values, (err, result) => {
      ...
      const pdfFolder = path.join(__dirname, "Spot-RM-PM_inspection_pdfs");
      if (!fs.existsSync(pdfFolder)) fs.mkdirSync(pdfFolder, { recursive: true });
      ...
      try {
        ...
        doc.moveDown(0.6);
        doc.fontSize(16).font("Helvetica-Bold")
          .text("VEHICLE INSPECTION CHECKLIST-Spot RM PM  ", { align: "center" });
...
doc.fillColor("black").font("Helvetica").fontSize(10);
        ...
        stream.on("finish", () => {
          db.commit((err) => {
            ...
            const pdfUrl = `http://localhost:3456/Spot-RM-PM_inspection_pdfs/inspection_${insertId}.pdf`; // Generate Only Spot Hired Vehicle
